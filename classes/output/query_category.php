...
/**
 * Query category renderable class.
 *
 * @package    report_datawarehouse
 * @copyright  2023 Luca Bösch <luca.boesch@bfh.ch>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class query_category implements renderable, templatable {
    /** @var \report_datawarehouse\local\query_category Category object. */
     ...
     */
    public function __construct(\report_datawarehouse\local\query_category $querycategory, context $context,
        bool $expandable = false, int $showquerycat = 0, int $hidequerycat = 0, bool $showonlythislink = false,
        bool $addnewquerybtn = true, moodle_url $returnurl = null) {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $queriesdata = $this->querycategory->get_queries_data();
    ...
    private function get_showing_state(): string {
        ...
        return $categoryid == $this->showquerycat && $categoryid != $this->hidequerycat ? 'shown' : 'hidden';
    }
    ...
    private function get_link_reference(): string {
        $categoryid = $this->querycategory->get_id();
        if ($categoryid == $this->showquerycat) {
            $params = ['hidequerycat' => $categoryid];
        } else {
            $params = ['showquerycat' => $categoryid];
        }
